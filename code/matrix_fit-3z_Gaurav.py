...
TDATA = range(T)
SVDCUT = 0.001
NEXP = 13                   # exponentials in fit
diag = 12                    # start fit from here
offdiag = 28                # fit for off diagonal elements (gl, lg etc.)
t0 = 9                      # initial timeslice to generate priors
tag = KEYFMT[:-8]           # with a .
ttag = tag[:-1]             # no .
otag = ttag + '_o.'         # oscillating tags

corr = 'Hy_m0.450_doublysmeared_lrho.txt'
file = os.path.join('proc_corrs', 'l3296f211b630m0074m037m440-coul-v5', corr)

#date = datetime.datetime.now()
log_name =  'LOG' + '_' + corr #str(date)       # name of log file.
...
def make_data(filename):
    data = -1*gv.dataset.avg_data(cf.read_dataset(filename, grep=tag[:-1]))
