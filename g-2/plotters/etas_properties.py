...
ax1.errorbar( [x.mean for x in xdata], [y.mean for y in m_etas], xerr=[x.sdev for x in xdata], yerr=[y.sdev for y in m_etas], fmt='h', color='red',lw=1, label='this work', alpha=0.8)
ax1.errorbar( 0, 689.89, yerr=0.49, color='blue', fmt='h', label='BMW (2020)', alpha=0.7)

ax2.errorbar( [x.mean for x in xdata], [y.mean for y in f_etas], xerr=[x.sdev for x in xdata], yerr=[y.sdev for y in f_etas], fmt='h', color='red',lw=1, label='this work', alpha=0.8)


ax1.set_xlim(left=-0.001)
ax2.set_xlabel('$a^2$[fm]', fontsize=25, labelpad=20)
ax1.set_ylabel('$M_{\eta_s}$\n[MeV]', fontsize=20, rotation=0, labelpad=40)
ax1.yaxis.set_label_coords(-0.2,0.25)
ax2.yaxis.set_label_coords(-0.2,0.25)
ax2.set_ylabel('$f_{\eta_s}$\n[MeV]', fontsize=20, rotation=0, labelpad=40)
ax1.legend(frameon=False,fontsize=12,loc='lower left')
#ax1.legend(frameon=False,fontsize=12,loc='upper left')

plt.tight_layout()
plt.savefig('../figures/etas_properties.png', dpi=500, bbox_inches="tight")
plt.show()
