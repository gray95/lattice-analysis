...
from commonweal import vc_rt, c_rt 
...
def fitargs(z):
    ...
    #dpp = z['dp1']
    prior = {}
    prior['a'] = gv.gvar([gv.gvar(1.0, dp), gv.gvar(0.1, dp)])
    return dict(prior=prior, fcn=extrap, data=([3, 5, 7], ydata))
...
print("prior width on a[0] that maxmises logGBF: ", z)
#print("prior width on a[1] that maxmises logGBF: ", z['dp1'])

#prior = {}
#prior['a'] = [gv.gvar('1(1)'),gv.gvar('0.0(1)')] #,gv.gvar(0,1),gv.gvar(0,1)]
...
plt.errorbar(3,ydata[0].mean,xerr=0,yerr=ydata[1].sdev,fmt='h',mfc='none',color='r',lw=1)
...
#plt.title('0.15fm ensemble - 356 configs')

#plt.ylim(top=0.00)
#plt.xlim(left=0, right=10)

plt.tight_layout()
#plt.savefig('../figures/amu_crt_chiExt.png', dpi=500)
plt.show()
